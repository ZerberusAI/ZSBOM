...
"""
        ...
        for pkg in high_risk_packages:
            ...
            markdown += f"| {pkg['name']} | {ecosystem_emoji} {pkg.get('ecosystem', 'unknown').title()} | {pkg.get('risk_score', 0):.1f} | {risk_level_emoji} {pkg.get('risk_level', 'unknown').upper()} | {vuln_str} |\n"
...
"""
    ...
    def _generate_footer(self) -> str:
        ...
        run_number = self.scan_metadata.get("ci_context", {}).get("run_id", "N/A")
...
:robot: *Generated by ZSBOM v{version} | Template v{template_version}*"""
        ...
        if run_number != "N/A":
            footer += f"  \n:link: *[Workflow Run #{run_number}](workflow-url)*"
