...
class TestTransitiveDependencyAnalysis:
    ...
    def test_extract_dependencies_with_transitive_analysis_no_pip_tools(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Should have dependencies but transitive analysis should indicate pip-tools unavailable
            assert "dependencies" in result
            assert "transitive_analysis" in result
            assert result["transitive_analysis"]["status"] == "pip_tools_unavailable"
    ...
    def test_extract_dependencies_with_transitive_analysis_no_requirements(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Should have empty dependencies and no_requirements status
            assert "dependencies" in result
            assert "transitive_analysis" in result
            assert result["transitive_analysis"]["status"] == "no_requirements"
...
class TestIntegrationWithConfig:
    ...
    def test_extract_dependencies_with_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Should return new format with both dependencies and transitive_analysis
    ...
    def test_backwards_compatibility_fallback(self):
        """Test that the function gracefully handles old format expectations."""
        with tempfile.TemporaryDirectory() as temp_dir:
            req_file = Path(temp_dir) / "requirements.txt"
            req_file.write_text("requests>=2.28.0")
